...
import {
  MessageCircle, Eye, ArrowRightLeft, ArrowRight, Mail, Star,
  MailOpen, SendHorizontal, Calendar, ClipboardList,
  PauseCircle, Trash2, ChevronDown, AlertCircle, CheckCircle2,
  TrendingUp, Zap
} from 'lucide-react';
...
const NQLLeadCard: React.FC<NQLLeadCardProps> = ({ lead, onStatusChange }) => {
  ...
  // 'MQL' | 'SQL' = first confirmation click; null = not confirming
  ...
  // Derive executed state from persistent lead fields (survives reloads)
  const isExecuted = (
  ...
  // --- Action indicator logic ---
  // The DATE is the source of truth: if there's no fecha, there's no task — no block, no button.
  ...
  // Convert to MQL or SQL: changes quality and removes from NQL pipeline
  ...
  // Next scheduled date chip for Landing column
  ...
  // Border color by source
  const borderClass = lead.source === 'APOLLO'
    ? 'border-l-4 border-l-amber-400'
    : lead.source === 'PRECOTIZADOR'
  ...
  return (
    <>
      <div className={`bg-white rounded-xl shadow-sm border border-slate-100 ${borderClass} ${opacityClass} hover:shadow-md transition-shadow duration-200 text-xs`}>
        ...
        <div className="flex items-start justify-between px-3 pt-3 pb-1 gap-1">
          ...
          <div className="flex-1 min-w-0 mx-1">
            ...
            {(lead.cargo || lead.compania) && (
              <p className="text-slate-400 truncate text-[10px]">{lead.cargo}{lead.cargo && lead.compania ? ' · ' : ''}{lead.compania}</p>
            )}
          </div>
          <button
        ...
        {(lead.origen || lead.destino) && (
          <div className="mx-3 mb-2 px-2 py-1.5 bg-slate-50 rounded-lg flex items-center gap-1 text-slate-600">
            {lead.origen && <span className="truncate">{lead.origen}</span>}
            {lead.origen && lead.destino && (
              lead.fechaRegreso
                ? <ArrowRightLeft size={11} className="shrink-0 text-slate-400" />
                : <ArrowRight size={11} className="shrink-0 text-slate-400" />
            )}
            {lead.destino && <span className="truncate font-medium">{lead.destino}</span>}
          </div>
        )}
        ...
        {lead.valor && (
          <div className="mx-3 mb-2 text-right">
            <span className="font-semibold text-slate-700">{formatDisplayValue(lead.valor)}</span>
          </div>
        )}

        {/* === ACTION INDICATOR (Seg 1/2/3) === */}
        {actionInfo && (
          <div className={`mx-3 mb-2 p-2 rounded-lg border ${
            ...
            {/* Marcar ejecutado: persiste en la hoja. NO cambia de etapa.
                La etapa avanza automáticamente cuando llega la fecha del siguiente seguimiento. */}
            {!isExecuted && (
              <button
                onClick={() => {
          ...
          <button
            ...
            className={`p-1.5 rounded-lg transition-colors relative ${
              lead.emailOpened === true
                ? 'text-orange-500 bg-orange-50 ring-1 ring-orange-200 animate-pulse'
                : lead.emailOpened === 'SENT'
                ? 'text-indigo-500 bg-indigo-50'
                : 'text-blue-500 hover:bg-blue-50'
            }`}
            title={lead.emailOpened === true ? 'Email leído — clic para limpiar' : lead.emailOpened === 'SENT' ? 'Enviado' : 'Enviar email'}
          >
            {lead.emailOpened === true ? <MailOpen size={14} /> : lead.emailOpened === 'SENT' ? <SendHorizontal size={14} /> : <Mail size={14} />}
          </button>
              ...
              <div className="absolute bottom-full right-0 mb-1 w-52 bg-white border border-slate-200 rounded-xl shadow-xl z-50 overflow-hidden">
                ...
                {convertConfirm ? (
                  <div className="px-3 py-3">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 py-1.5 text-[11px] font-medium text-slate-600 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors"
                      >
                        Cancelar
                      </button>
                      <button
                  ...
                  <>
                    ...
                    {status !== NurturingStatus.PAUSADO && (
                      <button
                        onClick={() => { onStatusChange(lead.id, NurturingStatus.PAUSADO); setShowMenu(false); }}
                    ...
                    {status !== NurturingStatus.DESCARTADO && (
                      <button
                        onClick={() => { onStatusChange(lead.id, NurturingStatus.DESCARTADO); setShowMenu(false); }}
                    ...
                    {status !== NurturingStatus.LANDING && (
                      <button
                        onClick={() => { onStatusChange(lead.id, NurturingStatus.LANDING); setShowMenu(false); }}
                        className="w-full flex items-center gap-2 px-3 py-2 text-xs text-sky-600 hover:bg-sky-50"
                      >
      ...
      <EmailSelectionModal lead={lead} isOpen={isEmailOpen} onClose={() => setIsEmailOpen(false)} onSuccess={() => {}} />
