...
const NurturingScheduleModal: React.FC<NurturingScheduleModalProps> = ({ lead, onClose }) => {
  ...
  // Estado per seguimiento — 'Ejecutado' | 'Por Ejecutar' | ''
  ...
  // Locked only when the slot is empty AND the previous slot is also empty.
  // If the slot already has data, it's always editable (even if upstream was reset).
  ...
  // Estado toggle + reset controls for one seguimiento
  const SegControls = ({
    ...
    return (
      <div className="flex items-center justify-between gap-2 pt-1">
        ...
        {/* Reset button — disabled if already Ejecutado */}
        <button
          type="button"
          onClick={onReset}
          disabled={isExecuted}
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden">
          ...
          <div className="p-4 rounded-xl border border-amber-200 bg-amber-50/50 space-y-3">
            ...
            <div>
              ...
              <input
                type="date"
                value={fechaSeg1}
                onChange={e => setFechaSeg1(e.target.value)}
            ...
            <div>
              ...
              <textarea
                rows={2}
                value={accionSeg1}
                onChange={e => setAccionSeg1(e.target.value)}
                placeholder="ej: Llamar para recordar oferta de vuelo charter"
                className={textareaClass}
              />
            </div>
            <SegControls
              fecha={fechaSeg1} estado={estadoSeg1}
              locked={false} isExecuted={estadoSeg1 === 'Ejecutado'}
              onEstadoChange={setEstadoSeg1}
              onReset={() => { setFechaSeg1(''); setAccionSeg1(''); setEstadoSeg1(''); }}
            />
          </div>
          ...
          <div className={`p-4 rounded-xl border space-y-3 transition-opacity ${seg2Locked ? 'border-slate-200 bg-slate-50/50 opacity-60' : 'border-amber-100 bg-amber-50/20'}`}>
            ...
            <div>
              ...
              <input
                type="date"
                value={fechaSeg2}
                min={fechaSeg1 ? fechaSeg1 : undefined}
                disabled={seg2Locked}
                onChange={e => setFechaSeg2(e.target.value)}
                className={inputClass}
              />
            </div>
            <div>
              ...
              <textarea
                ...
                value={accionSeg2}
                disabled={seg2Locked}
                onChange={e => setAccionSeg2(e.target.value)}
                placeholder="ej: Enviar email con promo de temporada"
                className={textareaClass}
              />
            </div>
            <SegControls
              fecha={fechaSeg2} estado={estadoSeg2}
              locked={seg2Locked} isExecuted={estadoSeg2 === 'Ejecutado'}
              onEstadoChange={setEstadoSeg2}
              onReset={() => { setFechaSeg2(''); setAccionSeg2(''); setEstadoSeg2(''); }}
            />
          </div>
          ...
          <div className={`p-4 rounded-xl border space-y-3 transition-opacity ${seg3Locked ? 'border-slate-200 bg-slate-50/50 opacity-60' : 'border-slate-200 bg-slate-50/40'}`}>
            ...
            <div>
              ...
              <input
                type="date"
                value={fechaSeg3}
                min={fechaSeg2 ? fechaSeg2 : undefined}
                disabled={seg3Locked}
                onChange={e => setFechaSeg3(e.target.value)}
                className={inputClass}
              />
            </div>
            <div>
              ...
              <textarea
                ...
                value={accionSeg3}
                disabled={seg3Locked}
                onChange={e => setAccionSeg3(e.target.value)}
                placeholder="ej: WA de reactivación con newsletter"
                className={textareaClass}
              />
            </div>
            <SegControls
              fecha={fechaSeg3} estado={estadoSeg3}
              locked={seg3Locked} isExecuted={estadoSeg3 === 'Ejecutado'}
              onEstadoChange={setEstadoSeg3}
              onReset={() => { setFechaSeg3(''); setAccionSeg3(''); setEstadoSeg3(''); }}
            />
          </div>
        ...
        <div className="px-5 py-4 border-t border-slate-100 flex items-center justify-end gap-2">
          ...
          <button
            onClick={handleSave}
            className="flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-white bg-amber-500 rounded-lg hover:bg-amber-600 transition-colors"
          >
            <Save size={13} />
