...
const FUNNEL_STAGES = [
  ...
  { id: CrmStatus.NEGOCIACION, label: 'Negociación', color: 'bg-fuchsia-50 border-fuchsia-200 text-fuchsia-700' },
  { id: CrmStatus.GANADOS, label: 'Ganados', color: 'bg-green-50 border-green-200 text-green-700' }
];
...
const PipelineView: React.FC = () => {
  ...
  const currentMonthLeads = useMemo(() => {
      ...
      // For "Ganados", filtering by date is crucial.
      // For Active, we usually show everything that is currently open (Snapshot view)
      return leads.filter(l => {
          // 1. Logic for GANADOS (Won) - Strict Monthly Filter
          if (l.crmStatus === CrmStatus.GANADOS) {
               ...
               const [y, m, d] = l.fechaVenta.split('-').map(Number);
          ...
          if (l.crmStatus === CrmStatus.NUEVO) {
              ...
              // Only show leads created in the last 30 days for "Nuevos"
  ...
  const goals = useMemo(() => {
      ...
      const negotiationGoal = salesGoal * pipelineConfig.NEG_MULT;
      const quotedGoal = negotiationGoal * pipelineConfig.QUO_MULT; 

      // Actuals (Snapshot for Active, Closed for Won)
      const wonActual = funnelMetrics.funnelStats.find(s => s.id === CrmStatus.GANADOS)?.currentValue || 0;
      
      // FIX: Use currentValue (Snapshot) instead of accumulatedValue for specific stage goals
      // This ensures "Negociación" only shows what is CURRENTLY in negotiation, not including Won.
      const negActual = funnelMetrics.funnelStats.find(s => s.id === CrmStatus.NEGOCIACION)?.currentValue || 0;
      const quoActual = funnelMetrics.funnelStats.find(s => s.id === CrmStatus.COTIZADO)?.currentValue || 0;

      return {
          sales: { goal: salesGoal, actual: wonActual, percent: salesGoal > 0 ? (wonActual / salesGoal) * 100 : 0 },
          neg: { goal: negotiationGoal, actual: negActual, percent: negotiationGoal > 0 ? (negActual / negotiationGoal) * 100 : 0 },
          quo: { goal: quotedGoal, actual: quoActual, percent: quotedGoal > 0 ? (quoActual / quotedGoal) * 100 : 0 }
      };
    ...
    <div className="flex-1 overflow-y-auto bg-slate-50 p-4 md:p-8 animate-fade-in pb-20">
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {goals && (
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-200 relative overflow-hidden">
                    <div className="flex justify-between items-start mb-4">
                        <div className="p-2 bg-fuchsia-100 text-fuchsia-700 rounded-lg">
                         ...
                         <div className="text-right">
                             <span className="text-xs font-bold uppercase text-fuchsia-600 bg-fuchsia-50 px-2 py-0.5 rounded-full block mb-1">
                                Negociación
                             </span>
                    ...
                    <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden mb-2">
                        <div 
                             className={`h-full rounded-full transition-all duration-1000 ${goals.neg.percent >= 100 ? 'bg-fuchsia-500' : 'bg-fuchsia-400'}`}
                ...
                <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-200 relative overflow-hidden">
                    ...
                    <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden mb-2">
                        <div 
                             className={`h-full rounded-full transition-all duration-1000 ${goals.quo.percent >= 100 ? 'bg-indigo-500' : 'bg-indigo-400'}`}
        ...
        {funnelMetrics.funnelStats.map((stage, index) => {
            ...
            // Conversion Rate Calculation based on ACCUMULATED flow (Visual Funnel Logic remains)
            ...
            // Determine if we should show the "View List" button
            const isInteractiveStage = stage.id === CrmStatus.NEGOCIACION || stage.id === CrmStatus.GANADOS;
                ...
                <div key={stage.id} className="relative group">
                    ...
                    <div className="flex items-stretch bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow relative">
                        <div className={`w-32 md:w-48 p-4 flex flex-col justify-center border-r border-slate-100 ${stage.color} bg-opacity-10`}>
                            ...
                            {/* ACTION BUTTON FOR SPECIFIC STAGES */}
                            {isInteractiveStage && (
                                <button 
                        ...
                        <div className="w-40 md:w-56 p-4 flex flex-col justify-center items-end border-l border-slate-100 bg-white">
                            <div className="flex items-center gap-1.5 text-slate-800 font-bold text-lg md:text-xl">
                                ...
                                {stage.id === CrmStatus.NEGOCIACION ? stage.currentCount : stage.accumulatedCount}
                            </div>
                            <div className="flex items-center gap-1 text-slate-600 font-semibold text-sm mt-1">
                                ...
                                {new Intl.NumberFormat('es-CO', { notation: "compact", maximumFractionDigits: 1 }).format(
                                    stage.id === CrmStatus.NEGOCIACION ? stage.currentValue : stage.accumulatedValue
                                )}
                            </div>
                        </div>
                    </div>
                           ...
                           <table className="w-full text-left text-sm border-collapse">
                              <thead className="bg-slate-50 text-slate-500 font-bold uppercase text-[11px] tracking-wider border-b border-slate-200">
                                <tr>
                                    ...
                                    <th className="p-4 text-right">Fecha Vuelo</th>
                              ...
                              <tbody className="divide-y divide-slate-100">
                                {leads.map(lead => (
                                    <tr key={lead.id} className="hover:bg-blue-50/30 transition-colors">
                                        ...
                                        {/* Fecha Vuelo (Always Visible) */}
                                        <td className="p-4 text-right text-slate-600 font-medium text-xs">
                                            {lead.fecha || <span className="text-slate-300">-</span>}
                                        </td>

                                        {/* Fecha Venta (Conditional) */}
